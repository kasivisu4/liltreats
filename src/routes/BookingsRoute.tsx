...
import { useScoopBookings } from "../api/queries";
...
function BookingCard({ booking }: { booking: ScoopBooking }) {
  const isVideo = booking.experience === "with_video";
  const tierLabel =
    booking.scoopTier === "mini"
      ? "🌿 Mini Scoop"
      : booking.scoopTier === "magic"
        ? "✨ Magic Scoop"
        : "👑 Premium Scoop";
    ...
    <div className="card-glass mb-3 p-4">
      <div className="mb-2.5 flex items-start gap-3">
        ...
        <div className="flex-1">
          ...
          <div className="mt-0.5 text-[11px] font-semibold text-ink-mute">
            Booking {booking.id} · Order {booking.orderId}
          </div>
        </div>
        <span
          className={`rounded-[10px] px-2.5 py-1.5 text-[10px] font-bold ${
      ...
      <div className="space-y-1.5 rounded-xl bg-white/50 px-3 py-2.5 text-[12px]">
        ...
        <div className="flex justify-between border-t border-line pt-1.5">
          ...
          <span className="font-bold text-deep">{fmtDate(booking.createdAt)}</span>
        </div>
      </div>
      ...
      {isVideo && booking.videoDate && (
        <div className="mt-2.5 flex items-center gap-2 rounded-xl border border-rose/20 bg-[#FFF0F4] px-3 py-2">
          ...
          <p className="text-[11px] font-semibold text-rose">
            We'll tag you on @_liltreats_ on your video day!
          </p>
        </div>
      )}
    </div>
...
export function BookingsRoute() {
  const { data: bookings, isLoading } = useScoopBookings();
  const videoBookings = bookings?.filter((b) => b.experience === "with_video") ?? [];
  const normalBookings = bookings?.filter((b) => b.experience === "without_video") ?? [];
      ...
      <div className="p-4">
        {isLoading ? (
          ...
          <div className="pt-16 text-center">
            ...
            <h3 className="mt-3 font-serif text-[18px] font-semibold text-deep">No bookings yet</h3>
            <p className="mt-1.5 text-[13px] font-semibold text-ink-soft">
              When you book a scoop, it will appear here.
            </p>
          </div>
        ) : (
          <>
            {videoBookings.length > 0 && (
              <>
                <div className="mb-3 flex items-center gap-2">
                  ...
                  <span className="text-[12px] font-bold uppercase tracking-wide text-rose">
                    Video bookings ({videoBookings.length})
                  </span>
                </div>
                {videoBookings.map((b) => (
              ...
              <>
                <div className="mb-3 mt-2 flex items-center gap-2">
                  ...
                  <span className="text-[12px] font-bold uppercase tracking-wide text-gold">
                    Normal scoops ({normalBookings.length})
                  </span>
                </div>
