...
import { LayoutDashboard, ShoppingBag, CalendarDays, Video, Package, Boxes, Users, TrendingUp, ArrowLeft, Menu, X, BarChart2, CreditCard, Truck, Settings } from "lucide-react";
...
export function AdminLayout() {
  ...
  const isActive = (path: string) => path === "/admin/" ? pathname === "/admin" || pathname === "/admin/" : pathname.startsWith(path);

  const SidebarNav = ({ onNavigate }: { onNavigate?: () => void }) => (
    <nav className="flex-1 overflow-y-auto px-2 py-3">
      {NAV.map(({ path, label, icon: Icon, ...rest }) => {
        const section = "section" in rest ? rest.section : undefined;
        return (
          <div key={path}>
            {section && <div className="mb-1 mt-3 px-2 text-[9px] font-bold uppercase tracking-[1.5px] text-ink-mute first:mt-0">{section}</div>}
            <button onClick={() => { navigate({ to: path as "/" }); onNavigate?.(); }}
              className={`mb-0.5 flex w-full items-center gap-2.5 rounded-xl px-3 py-2.5 text-left text-[13px] font-bold transition-colors ${isActive(path) ? "bg-deep text-cream" : "text-ink-soft hover:bg-white/60 hover:text-deep"}`}>
              <Icon size={15} />{label}
            </button>
          </div>
        );
    ...
    <div className="flex h-[100dvh] bg-[#F4EDE8] text-ink">
      ...
      <aside className="hidden w-56 flex-shrink-0 flex-col border-r border-line bg-[#FBF6F0] md:flex">
        ...
        <div className="border-t border-line px-4 py-3">
          <button onClick={() => navigate({ to: "/" })} className="flex w-full items-center gap-2 text-[12px] font-bold text-ink-mute">
        ...
        <div className="fixed inset-0 z-50 md:hidden">
          ...
          <aside className="absolute left-0 top-0 flex h-full w-64 flex-col bg-[#FBF6F0]">
            <div className="flex items-center justify-between border-b border-line px-4 py-4">
              ...
              <button onClick={() => setMobileOpen(false)}><X size={18} className="text-ink-mute" /></button>
            </div>
      ...
      <div className="flex flex-1 flex-col overflow-hidden">
        <div className="flex items-center gap-3 border-b border-line bg-[#FBF6F0] px-4 py-3 md:hidden">
          <button onClick={() => setMobileOpen(true)}><Menu size={20} className="text-deep" /></button>
