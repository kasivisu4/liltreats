...
export function LoginRoute() {
  ...
  // Read ?redirect= query param so we can send user back after login
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      if (mode === "login") {
        ...
        // forgot password — mock: just show success
      ...
      setDone(true);
      setTimeout(() => navigate({ to: redirectTo as "/" }), 900);
    } catch (err) {
      ...
      <Screen top={<TopBar title="Reset password" showBack />}>
        <div className="flex flex-col items-center px-6 pt-20 text-center">
          ...
          <p className="mb-6 text-[13px] font-semibold leading-relaxed text-ink-soft">
            ...
            <span className="text-deep">{form.email}</span>. Follow the link to create a new password.
          </p>
          <button
            onClick={() => switchMode("login")}
            className="text-[13px] font-bold text-mauve underline underline-offset-2"
          >
            Back to sign in
          </button>
        </div>
  ...
  return (
    <Screen top={<TopBar title={mode === "forgot" ? "Reset password" : mode === "login" ? "Sign in" : "Create account"} showBack />}>
      <div className="px-5 pb-12 pt-6">
        ...
        {/* Tab toggle (not shown for forgot) */}
        {mode !== "forgot" && (
        ...
        <form onSubmit={handleSubmit} className="space-y-3.5">
                ...
                <FieldWrap label="Full name" error={fieldErrors.name}>
                  <div className="relative">
                    ...
                    <input
                      ...
                      className={`field-input !pl-10 ${fieldErrors.name ? "border-[#F0C0C0]" : ""}`}
                      autoComplete="name"
                    />
                  </div>
                </FieldWrap>
                ...
                <FieldWrap label="WhatsApp / Phone" error={fieldErrors.phone}>
                  <div className="relative">
                    ...
                    <input
                      ...
                      className={`field-input !pl-10 ${fieldErrors.phone ? "border-[#F0C0C0]" : ""}`}
                      autoComplete="tel"
                    />
                  </div>
                </FieldWrap>
          ...
          <FieldWrap label="Email address" error={fieldErrors.email}>
            <div className="relative">
              ...
              <input
                ...
                className={`field-input !pl-10 ${fieldErrors.email ? "border-[#F0C0C0]" : ""}`}
                autoComplete="email"
              />
            </div>
          </FieldWrap>
            ...
            <FieldWrap label="Password" error={fieldErrors.password}>
              <div className="relative">
                ...
                <input
                  ...
                  placeholder="••••••••"
                  className={`field-input !pl-10 !pr-10 ${fieldErrors.password ? "border-[#F0C0C0]" : ""}`}
          ...
          <button
            ...
            className={`btn-main mt-2 flex w-full items-center justify-center gap-2 transition-all ${
              done ? "bg-[#2A6030]" : ""
            }`}
          >
            {done ? (
        ...
        {mode !== "forgot" && (
          <>
            ...
            <div className="grid grid-cols-2 gap-2">
              <button className="flex items-center justify-center gap-2 rounded-xl border border-line bg-white/70 py-3 text-[13px] font-bold text-deep transition-colors active:bg-white">
